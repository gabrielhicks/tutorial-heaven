...
function CategoryChat({
                ...
                },
                {
                    ...
                    received: (message) => {
                        setMessages((chatMessages) => ({
                            ...chatMessages,
                            feed: [...chatMessages.feed, message],
                        }));
                    },
                }
            );
    ...
    return (
        <motion.div
            ...
            <Link to={`/${categoryMessages.root_url}`}>
                <ChatImage src={`${image}`} />
            </Link>
            <ChatBoxContainer>
                <ChatTextarea>
                    <ChatWindow>
                        {chatMessages.feed ? (
                            <>
                                <div>
                                    {chatMessages.feed.map((message) => (
                                        <Message
                                            date={fixDate(message.created_at)}
                                            image={message.user.image}
                                            first={message.user.first_name}
...
const mapDispatchToProps = (dispatch) => {
    return {
        fetchCategoryMessages: (category) =>
            dispatch(fetchCategoryMessages(category)),
