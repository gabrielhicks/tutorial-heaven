    ...
    return category.posts.map((post) => (
      <GridItem key={post.id} item xs={10}>
        <PostCard
          ...
          <PostCardGrid>
            <Link style={{ width: '100px' }} to={`/${root}/${post.id}`}>
              <PostCardImage
                style={{ width: '100px' }}
                src={post.image_url}></PostCardImage>
            </Link>
            <PostCardTitle>
              <PostLink to={`/${root}/${post.id}`}>
                <h3>{post.title}</h3>
              </PostLink>
            </PostCardTitle>
            <PostCardAuthor>
              Posted by&nbsp;
              <Link style={{ color: 'black' }} to={`/profile/${post.user.id}`}>
                <i>{post.user.username}</i>
              </Link>
            </PostCardAuthor>
            <PostCardDate>{fixDate(post.created_at)}</PostCardDate>
            <PostCardComments>
              <Link style={{ color: 'black' }} to={`/${root}/${post.id}`}>
            ...
            {post.status === true ? (
              <PostCardStatus>
                {post.difficulty}&nbsp;|&nbsp;
                <i className='status active'>Open</i>
              </PostCardStatus>
            ) : (
              <PostCardStatus>
                {post.difficulty}&nbsp;|&nbsp;
                <i className='status'>Closed</i>
              </PostCardStatus>
            )}
          </PostCardGrid>
        </PostCard>
                      ...
                      <motion.div
                        ...
                        {newClicked === true ? (
                          <>
                            ...
                            <MyForm>
                              <NewPost
                                onClickHandler={newClickHandler}
                                categoryObj={category}
                              />
                            </MyForm>
                            <MaskDiv onClick={newClickHandler} />
                          </>
                        ) : null}
                        {renderIcons(root)}
                      </motion.div>
                          ...
                          {user.id ? (
                            <>
                              <SideBar container item xs={4}>
                                <NewPostStyle onClick={newClickHandler}>
                                  <AddCircleIcon />
                                  New
                                </NewPostStyle>
                                <NewPostStyle
